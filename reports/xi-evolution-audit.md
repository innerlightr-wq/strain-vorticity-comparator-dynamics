...
[`../docs/LITERATURE_REVIEW.md`](../docs/LITERATURE_REVIEW.md) §§6–7; records:
[`../references.bib`](../references.bib).

* **The two sector equations are standard.** The coupled vorticity / rate-of-strain
  dynamics in exactly this decomposition are studied by Nomura & Post (1998), and the
  strain equation with `−2tr S³`, `−P/2`, `−2S:H` and viscous terms is textbook
  (Meneveau 2011; Johnson & Wilczek 2024). The pressure-blindness of the vorticity
  sector is `curl grad p = 0`.
* **The closed form of `Dξ/Dt` is therefore a new derivation of known ingredients** — an
  immediate combination of two standard equations, half the difference of their
  logarithmic rates. It was not found written explicitly in the reviewed sources, but it
  is not a structurally new equation and must not be presented as one.
* **The local/nonlocal split of `H` is classical.** The decomposition into a local
  isotropic part fixed by `Δp` and a nonlocal deviatoric part is what restricted Euler
  discards (Vieillefosse 1982; Cantwell 1992), and Buaria & Pumir (2023) state it in
  exactly these terms and measure the two parts' opposite effects on vortex stretching.
  Yang et al. (2024) give an approximate closed form for `H` in strong-vorticity regions.
* **"A single scalar obstruction" is coordinate compression, and this report's framing
  of it as the sharpest positive structural result is withdrawn.** Any scalar observable
  sees exactly one contraction of `H`, so reducing to one scalar reflects the choice of a
  scalar observable, not a reduction of the pressure Hessian's complexity. The genuine
  rank reduction of the anisotropic pressure Hessian is Carbone, Iovieno & Bragg (2020),
  who use a gauge symmetry to confine its dynamical activity to two-dimensional
  manifolds — a substantially stronger statement about the tensor. Wilczek & Meneveau
  (2014) and Vlaykov & Wilczek (2019) give the statistical and structural background.
* Verdict **B** stands: the separation is exact and the nonlocal term is not controlled.
  What changes is the attribution of the ingredients, not the mathematics; all 49
  symbolic and 8 DNS checks are unaffected.
